...
void KLogger::open(const char* comm, const char* type, const char* mission) {
	m_mu_log->lock();
...
void KLogger::change_to_cont_file(KString date, KString filename) {
	m_mu_log->lock();

	assert(m_fid != NULL);

	KString dir = KArgs::log_root;
	KString filepath = dir + date + "/" + filename;
...
void KLogger::setCallback(void* pInst, KCbPrint* pcbFunc) {
	m_pCbInst = pInst;
	m_pcbFunc = pcbFunc;
}

void KLogger::PrintWait(const char* fmt, ...) {
	m_mu_log->lock();
...
void KLogger::Print(const char* fmt, ...) {
	m_mu_log->lock();
...
void KLogger::PrintLogWait(const char* fmt, ...) {
	m_mu_log->lock();
...
void KLogger::PrintLog(const char* fmt, ...) {
	m_mu_log->lock();
...
void KLogger::Bookeep(const char* fmt, ...) {
	m_mu_log->lock();
