...
#include "../../src2020//core/common.h"

#define kcuda_call(funcname, size, args) m_lock(); funcname << <(unsigned int)((size + m_block_size-1) / m_block_size), m_block_size>> > args; m_unlock()
#define kcuda_call_no_lock(funcname, size, args) funcname << <(unsigned int)((size + m_block_size-1) / m_block_size), m_block_size>> > args

class KaiDeviceManager;

class KaiCudaMath : public KaiHostMath {
public:
	KaiCudaMath(KaiDeviceManager* pDevManager);
	virtual ~KaiCudaMath();
	...
	virtual KaiArray<KFloat> mean(KaiArray<KFloat> arr);
	...
	KInt getDevAllocSize() { return m_pDevManager->getAllocSize(); }
	...
	List lstm_forward_test(KaiArray<KFloat> xarr, KaiArray<KFloat> yarr, KaiDict pm);
	void lstm_backprop_test(KaiArray<KFloat> gyarr, KaiArray<KFloat> gxarr, KaiDict pm, List aux);

protected:
	void m_lock();
	void m_unlock();
	
	KaiValue m_to_host(KaiValue value);

	virtual KInt m_getMaxBatchSize(KInt nNeedSizePerbatch);
	void m_fft_core_split(KFloat* cuda_buf1, KFloat* cuda_buf2, KFloat* cuda_y, KInt data_num, KInt freq_cnt, KInt dsize, KInt bsize, KInt split);

protected:
	KaiDeviceManager* m_pDevManager;
	int m_block_size;
};
