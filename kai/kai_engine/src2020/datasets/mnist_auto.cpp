...
*/
#include "mnist_auto.h"

MnistAutoDataset::MnistAutoDataset(float ratio)
	: AutoencodeDataset("mnist_auto", "classify", ratio) {
...
MnistAutoDataset::~MnistAutoDataset() {
}

void MnistAutoDataset::gen_plain_xdata(int64 nth, int64 data_idx, int64 xsize, float* px, Value& to_y) {
	assert(xsize == 28 * 28);
...
void MnistAutoDataset::gen_plain_ydata(int64 nth, int64 data_idx, int64 ysize, float* py, Value from_x) {
	assert(ysize == 10);
...
void MnistAutoDataset::visualize(Value xs, Value cest, Value cans) {
	m_show_select_results(cest, cans, m_target_names);
}

void MnistAutoDataset::visualize_autoencode(Dict xs, Dict code, Dict repl, Dict outs, Dict ys) {
	...
	m_draw_images_horz(hstack, Shape(28, 28), 5);
}

void MnistAutoDataset::visualize_hash(Array<int64> rank1, Array<int64> rank2, Array<int64> key_labels, Array<int64> dat_label, Array<float> distance, Array<float> keys, Array<float> repl, Array<float> xs) {

	int64 nkey = rank1.axis_size(0);

	Array<float> images(Shape(2 * nkey, 12, 784));
	...
	for (int64 n = 0; n < nkey; n++) {
		memcpy(iip + n * 2 * 12 * 784, kip + n * 784, sizeof(float) * 784);
		memcpy(iip + (n * 2 + 1) * 12 * 784, rip + n * 784, sizeof(float) * 784);
		...
		for (int64 m = 0; m < 10; m++) {
			...
			memcpy(iip + (n * 2 * 12 + 2 + m) * 784, xip + idx1 * 784, sizeof(float) * 784);
			memcpy(iip + ((n * 2 + 1) * 12 + 2 + m) * 784, xip + idx2 * 784, sizeof(float) * 784);
	...
	m_draw_images_horz(images, Shape(28, 28), 5);
}
