...
Yolo3Mission::Yolo3Mission(KHSession hSession, KString sub_model, enum Ken_test_level testLevel) : Mission(hSession, testLevel), m_feeder(sub_model), m_reporter(sub_model) {
	m_sub_model = sub_model;
}

Yolo3Mission::~Yolo3Mission() {
}

void Yolo3Mission::Execute() {
	//srand(1234);
	m_createComponents();
	m_execute();
}

void Yolo3Mission::m_createComponents() {
	KERR_CHK(KAI_Dataset_create(m_hSession, &m_hDataset, "feeding", { {"name", "yolo3 coco dataset"} }));
...
KHNetwork Yolo3Mission::m_buildNetwork() {
	...
	KERR_CHK(KAI_Network_create(m_hSession, &hDBL, "serial", { }));
	KERR_CHK(KAI_Network_append_named_layer(m_hSession, hDBL, "conv", { {"ksize", 3}, {"chn", "#chn"}, {"stride", "#stride"}, {"actfunc", "relu"} })); // l2_decay : 0.0005 미반영 상태임
	KERR_CHK(KAI_Network_append_named_layer(m_hSession, hDBL, "batchnormal", { {"decay", 0.999}, {"epsilon", 1e-5}, {"scale", true} }));
	KERR_CHK(KAI_Network_append_named_layer(m_hSession, hDBL, "activate", { {"actfunc", "leaky_relu"}, {"leaky_alpha", 0.1} }));
	KERR_CHK(KAI_Network_regist_macro(m_hSession, hDBL, "DBL"));

	KERR_CHK(KAI_Network_create(m_hSession, &hResUnit, "add", { "subnet", "serial" }));
